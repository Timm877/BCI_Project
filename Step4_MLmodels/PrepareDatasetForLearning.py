    ...
    def split_single_dataset_classification(self, dataset, class_labels, matching, split_fracs, filter=True, temporal=False, random_state=0):
        ...
        # For non temporal data we use a standard function to randomly split the dataset.
            ...
            training_set_X, test_set_X, training_set_y, test_set_y = train_test_split(dataset.iloc[:,features],
                                                                                        dataset.iloc[:,class_label_indices], test_size=split_fracs[0], stratify=dataset.iloc[:,class_label_indices], random_state=random_state)
            training_set_X, val_set_X, training_set_y, val_set_y = train_test_split(training_set_X,
                                                                                        training_set_y, test_size=split_fracs[1], stratify=training_set_y, random_state=random_state)                                                                           
        return training_set_X, val_set_X, test_set_X, training_set_y, val_set_y, test_set_y
        ...
        if unknown_users:
            ...
            for i in range(0, training_len):
                ...
                training_set_X_person, test_set_X_person, training_set_y_person, test_set_y_person = self.split_single_dataset_classification(datasets[indices[i]], class_labels, matching,
                                                                                                                                              [1,1], filter=filter, temporal=temporal, random_state=random_state)
            ...
            for j in range(training_len, len(datasets)):
                ...
                training_set_X_person, test_set_X_person, training_set_y_person, test_set_y_person = self.split_single_dataset_classification(
                    datasets[indices[j]], class_labels, matching, 1, filter=filter, temporal=temporal, random_state=random_state)
                # We add a person column.
            ...
            for i in range(0, len(datasets)):
                ...
                val_set_X = self.update_set(training_set_X, val_set_X_person)
                val_set_y = self.update_set(training_set_y, val_set_y_person)
